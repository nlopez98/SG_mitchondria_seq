...
params.src = "/home/ubuntu/SG_mitchondria_seq/src"
...
process mapping {
	...
	output:
	tuple val(sample_id), path('*.bam') into ch_minimap_bam
...
process samtools_preprocessing {
	...
	output:
	tuple val(sample_id), path("*_all.sorted.bam"), path("*_no_secondary_supplementary.sorted.bam") into ch_sorted_bam
...
process samtools_results {
	...
	input:
	tuple val(sample_id), path(sorted_bam), path(no_sec_supp_bam) from ch_sorted_bam

	output:
	tuple val(sample_id), path("*.stats"), path("*.flagstat"), path("*.idxstats") into ch_results
